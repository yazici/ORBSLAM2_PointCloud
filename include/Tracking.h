...
public:
    ...
    eTrackingState mMinjie;
    eTrackingState mLastProcessedState;

    // Input sensor
    int mSensor;

   //Input Keyframe_index
    int mKFIndex;
...
protected:
   ...
   //Linminjie Modified
    ...
    void HistoryTrack();
    void SavePose();
    cv::Mat dcm2quat(cv::Mat dcm );
    cv::Mat buildTransformMatrix_QT(cv::Mat q,cv::Mat t);

    cv::Mat quat2dcm(cv::Mat qnb);
    int sign (float i);
