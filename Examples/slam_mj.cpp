...
//    double fx = atof(pd.getData( "camera.fx" ).c_str());
//    double fy = atof(pd.getData( "camera.fy" ).c_str());
//    double cx = atof(pd.getData( "camera.cx" ).c_str());
//    double cy = atof(pd.getData( "camera.cy" ).c_str());
//    double camera_factor = atof(pd.getData( "camera_factor" ).c_str());
//    camera_instrics camera_info(fx,fy,cx,cy);
//    cout<<"camera intrics:: \n"<<camera_info.intrics<<endl;
/*initialization*/
...
/*tips : rgbfile format CV_8UC3*/
      ...
      string  depthfilename=getnextdepth(find,datasetpath);
       ...
       SLAM.TrackRGBD(rgb,depth,3000-count);
